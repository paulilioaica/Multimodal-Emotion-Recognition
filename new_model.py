...
class Classifier(nn.Module):
    ...
    def forward(self, audio, video_arr, motion_arr):
        if torch.cuda.is_available():
            h_video = torch.zeros((video_arr.shape[0], 128)).cuda()
            h_kinect = torch.zeros((video_arr.shape[0], 128)).cuda()
        else:
            h_video = torch.zeros((video_arr.shape[0], 128))
            h_kinect = torch.zeros((video_arr.shape[0], 128))
